...
export class AdminService {
  ...
  async notifications(userId: string) {
    ...
    const [assets, maintenance, recent] = await Promise.all([
    ...
    const creates = [];
    for (const asset of assets) { const message = `Garansi ${asset.code} berakhir ${asset.warrantyUntil?.toLocaleDateString('id-ID')}`; if (!recent.some((row) => row.type === 'WARRANTY' && row.message === message)) creates.push(this.db.notification.create({ data: { userId, type: 'WARRANTY', title: 'Garansi segera berakhir', message } })); }
