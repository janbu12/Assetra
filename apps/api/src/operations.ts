...
export class OperationsService {
  ...
  async createMaintenance(dto: MaintenanceDto, actorId: string) { return this.db.$transaction(async (tx) => { const log = await tx.maintenanceLog.create({ data: { ...dto, createdById: actorId } }); await tx.asset.update({ where: { id: dto.assetId }, data: { status: 'MAINTENANCE' } }); return log; }); }
  async updateMaintenance(id: string, status: MaintenanceStatus, action?: string) {
    const log = await this.db.maintenanceLog.update({ where: { id }, data: { status, action, completedAt: status === 'COMPLETED' ? new Date() : undefined }, include: { asset: true } });
    if (status === 'COMPLETED') await this.db.asset.update({ where: { id: log.assetId }, data: { status: log.asset.condition === 'GOOD' ? 'AVAILABLE' : 'MAINTENANCE' } });
    return log;
  }
  ...
  async scan(id: string, dto: AuditScanDto, actorId: string) {
    ...
    await this.db.$transaction(async (tx) => {
      ...
      await tx.auditResult.upsert({ where: { sessionId_assetId: { sessionId: id, assetId: asset.id } }, create: { sessionId: id, assetId: asset.id, status: observedStatus, observedLocation: dto.location, observedCondition: dto.condition, note: dto.note, scannedAt: new Date(dto.scannedAt), scannedById: actorId }, update: { status: observedStatus, observedLocation: dto.location, observedCondition: dto.condition, note: dto.note, scannedAt: new Date(dto.scannedAt), scannedById: actorId } });
    });
  ...
  completeAudit(id: string) { return this.db.auditSession.update({ where: { id }, data: { status: AuditStatus.COMPLETED, completedAt: new Date() }, include: { results: true } }); }
  listAudits() { return this.db.auditSession.findMany({ include: { _count: { select: { results: true, events: true } } }, orderBy: { createdAt: 'desc' } }); }
}
...
export class OperationsController {
  ...
  @Permissions('maintenance.manage') @Patch('maintenance/:id') updateMaintenance(@Param('id') id: string, @Body('status') status: MaintenanceStatus, @Body('action') action?: string) { return this.service.updateMaintenance(id, status, action); }
